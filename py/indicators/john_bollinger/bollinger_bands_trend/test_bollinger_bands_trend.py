"""Tests for Bollinger Bands Trend indicator."""

import math
import unittest
from datetime import datetime

from py.indicators.john_bollinger.bollinger_bands_trend.bollinger_bands_trend import BollingerBandsTrend
from py.indicators.john_bollinger.bollinger_bands_trend.params import BollingerBandsTrendParams
from py.indicators.john_bollinger.bollinger_bands_trend.output import BollingerBandsTrendOutput
...
class TestBollingerBandsTrendSampleFullData(unittest.TestCase):
    """Test BBTrend with sample (unbiased) standard deviation."""

    def test_sample_stddev_full_data(self):
        closing_price = test_closing_price()
        expected = test_sample_expected()

        ind = BollingerBandsTrend(BollingerBandsTrendParams(
            fast_length=20, slow_length=50, is_unbiased=True))

        for i in range(252):
            v = ind.update(closing_price[i])
            if math.isnan(expected[i]):
                self.assertTrue(math.isnan(v), f"[{i}] expected NaN, got {v}")
            else:
                self.assertAlmostEqual(v, expected[i], delta=1e-8,
                                       msg=f"[{i}] expected {expected[i]}, got {v}")


class TestBollingerBandsTrendPopulationFullData(unittest.TestCase):
    """Test BBTrend with population standard deviation."""

    def test_population_stddev_full_data(self):
        closing_price = test_closing_price()
        expected = test_population_expected()

        ind = BollingerBandsTrend(BollingerBandsTrendParams(
            fast_length=20, slow_length=50, is_unbiased=False))
...
class TestBollingerBandsTrendIsPrimed(unittest.TestCase):
    """Test is_primed behavior."""

    def test_is_primed(self):
        ind = BollingerBandsTrend(BollingerBandsTrendParams(
            fast_length=20, slow_length=50))
        closing_price = test_closing_price()

        self.assertFalse(ind.is_primed())

        for i in range(49):
            ind.update(closing_price[i])
            self.assertFalse(ind.is_primed(), f"[{i}] expected not primed")

        ind.update(closing_price[49])
        self.assertTrue(ind.is_primed(), "expected primed after index 49")


class TestBollingerBandsTrendNaN(unittest.TestCase):
...
class TestBollingerBandsTrendMetadata(unittest.TestCase):
    ...
    def test_metadata(self):
        ...
        self.assertEqual(meta.identifier, Identifier.BOLLINGER_BANDS_TREND)
        self.assertEqual(len(meta.outputs), 1)
        self.assertEqual(meta.outputs[0].kind, int(BollingerBandsTrendOutput.VALUE))


class TestBollingerBandsTrendUpdateScalar(unittest.TestCase):
    """Test update_scalar method."""
    ...
    def test_update_scalar(self):
        ...
        for i in range(49):
            out = ind.update_scalar(Scalar(time=tm, value=closing_price[i]))
            v = out[0].value
            self.assertTrue(math.isnan(v), f"[{i}] expected NaN scalar, got {v}")

        # Feed index 49 — first primed value.
        out = ind.update_scalar(Scalar(time=tm, value=closing_price[49]))
        v = out[0].value
        self.assertAlmostEqual(v, expected[49], delta=1e-8,
                               msg=f"[49] expected {expected[49]}, got {v}")


class TestBollingerBandsTrendDefaultParams(unittest.TestCase):
    """Test default parameters (fast=20, slow=50)."""
...
class TestBollingerBandsTrendInvalidParams(unittest.TestCase):
    """Test invalid parameter validation."""

    def test_fast_length_too_small(self):
        with self.assertRaises(ValueError):
            BollingerBandsTrend(BollingerBandsTrendParams(fast_length=1, slow_length=50))

    def test_fast_length_negative(self):
        with self.assertRaises(ValueError):
            BollingerBandsTrend(BollingerBandsTrendParams(fast_length=-1, slow_length=50))

    def test_slow_length_too_small(self):
        with self.assertRaises(ValueError):
            BollingerBandsTrend(BollingerBandsTrendParams(fast_length=20, slow_length=1))

    def test_slow_length_negative(self):
        with self.assertRaises(ValueError):
            BollingerBandsTrend(BollingerBandsTrendParams(fast_length=20, slow_length=-1))

    def test_slow_not_greater_than_fast(self):
        with self.assertRaises(ValueError):
            BollingerBandsTrend(BollingerBandsTrendParams(fast_length=20, slow_length=20))

    def test_slow_less_than_fast(self):
        with self.assertRaises(ValueError):
            BollingerBandsTrend(BollingerBandsTrendParams(fast_length=50, slow_length=20))
