...
class DirectionalMovementPlus(Indicator):
    """Welles Wilder's Directional Movement Plus indicator.
    ...
    def __init__(self, p: DirectionalMovementPlusParams) -> None:
        length = p.length
        if length < 1:
    ...
    def metadata(self) -> Metadata:
        return build_metadata(
            Identifier.DIRECTIONAL_MOVEMENT_PLUS,
            "+dm",
    ...
    def update(self, high: float, low: float) -> float:
        ...
        if self._no_smoothing:
            ...
            if self._primed:
                ...
                if delta_plus > 0 and delta_plus > delta_minus:
                    self._accumulator += -self._accumulator / self._length + delta_plus
                else:
