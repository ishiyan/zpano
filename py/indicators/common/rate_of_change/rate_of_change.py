...
from .params import RateOfChangeParams


class RateOfChange(Indicator):
    """Computes the Rate of Change (ROC).

    ROCi = 100 * (Pi / Pi-l - 1), where l is the length.

    The indicator is not primed during the first l updates.
    """

    def __init__(self, params: RateOfChangeParams) -> None:
        length = params.length
        if length < 1:
            raise ValueError(
                "invalid rate of change parameters: length should be positive")
        ...
        mnemonic = f"roc({length}{component_triple_mnemonic(bc, qc, tc)})"
        description = f"Rate of Change {mnemonic}"
