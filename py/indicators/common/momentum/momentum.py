...
class Momentum(Indicator):
    ...
    MOMi = Pi - Pi-l, where l is the length.
    ...
    def __init__(self, params: MomentumParams) -> None:
        ...
        if length < 1:
            raise ValueError(
                "invalid momentum parameters: length should be positive")
        ...
        mnemonic = f"mom({length}{component_triple_mnemonic(bc, qc, tc)})"
        description = f"Momentum {mnemonic}"
