"""Rate of Change Percent indicator."""
...
from .params import RateOfChangePercentParams


class RateOfChangePercent(Indicator):
    """Computes the Rate of Change Percent (ROCP).

    ROCPi = Pi / Pi-l - 1, where l is the length.

    The indicator is not primed during the first l updates.
    """

    def __init__(self, params: RateOfChangePercentParams) -> None:
        length = params.length
        if length < 1:
            raise ValueError(
                "invalid rate of change percent parameters: length should be positive")
        ...
        mnemonic = f"rocp({length}{component_triple_mnemonic(bc, qc, tc)})"
        description = f"Rate of Change Percent {mnemonic}"
    ...
    def metadata(self) -> Metadata:
        return build_metadata(
            Identifier.RATE_OF_CHANGE_PERCENT,
            self._line.mnemonic,
