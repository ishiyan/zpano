"""Maximum Entropy Spectrum heatmap indicator."""
...
_DEF_DEGREE = 30
_DEF_MIN_PERIOD = 2.0
_DEF_MAX_PERIOD = 59.0
_DEF_SPECTRUM_RESOLUTION = 1
_DEF_AGC_DECAY_FACTOR = 0.995
_AGC_DECAY_EPSILON = 1e-12


def _build_flag_tags(p: Params, agc_on: bool, floating_norm: bool) -> str:
    s = ""
    if not agc_on:
        s += ", no-agc"
...
class MaximumEntropySpectrum:
    ...
    def __init__(self, params: Params) -> None:
        ...
        cfg_degree = params.degree if params.degree != 0 else _DEF_DEGREE
        ...
        if cfg_degree <= 0 or cfg_degree >= cfg_length:
            raise ValueError(f"{invalid}: Degree should be > 0 and < Length")
        if cfg_min_period < 2:
        ...
        flags = _build_flag_tags(resolved, agc_on, floating_norm)
        self.mnemonic = f"mespect({cfg_length}, {cfg_degree}, {cfg_min_period:g}, " \
            f"{cfg_max_period:g}, {cfg_resolution}{flags}{comp_mn})"
        self.description = "Maximum entropy spectrum " + self.mnemonic

        self._estimator = _Estimator(
            cfg_length, cfg_degree, cfg_min_period, cfg_max_period, cfg_resolution,
            agc_on, cfg_agc_decay,
        )
    ...
    def metadata(self) -> Metadata:
        return build_metadata(
            Identifier.MAXIMUM_ENTROPY_SPECTRUM,
            self.mnemonic,
            self.description,
