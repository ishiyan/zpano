...
from .params import TripleExponentialMovingAverageLengthParams, TripleExponentialMovingAverageSmoothingFactorParams
...
class TripleExponentialMovingAverage(Indicator):
    ...
    def __init__(self, length: int, smoothing_factor: float,
                 length2: int, length3: int, first_is_average: bool,
    ...
    @staticmethod
    def from_length(params: TripleExponentialMovingAverageLengthParams) -> 'TripleExponentialMovingAverage':
        return _new_tema(params.length, math.nan, params.first_is_average,
                         params.bar_component, params.quote_component, params.trade_component)

    @staticmethod
    def from_smoothing_factor(params: TripleExponentialMovingAverageSmoothingFactorParams) -> 'TripleExponentialMovingAverage':
        return _new_tema(0, params.smoothing_factor, params.first_is_average,
                         params.bar_component, params.quote_component, params.trade_component)
    ...
    def metadata(self) -> Metadata:
        return build_metadata(
            Identifier.TRIPLE_EXPONENTIAL_MOVING_AVERAGE,
            self._line.mnemonic,
        ...
        if self._first_is_average:
            if self._count == 1:
                ...
                if self._length3 == self._count:
                    ...
                    return 3.0 * (self._ema1 - self._ema2) + self._ema3
        else:  # Metastock
            if self._count == 1:
                self._ema1 = sample
            elif self._length >= self._count:
                ...
                self._ema1 += (sample - self._ema1) * self._smoothing_factor
                self._ema2 += (self._ema1 - self._ema2) * self._smoothing_factor
                self._ema3 += (self._ema2 - self._ema3) * self._smoothing_factor
                if self._length3 == self._count:
                    self._primed = True
                    return 3.0 * (self._ema1 - self._ema2) + self._ema3
...
def _new_tema(length: int, alpha: float, first_is_average: bool,
              bc_param, qc_param, tc_param) -> TripleExponentialMovingAverage:
    ...
    if math.isnan(alpha):
        ...
        alpha = 2.0 / (1 + length)
        mnemonic = f"tema({length}{component_triple_mnemonic(bc, qc, tc)})"
    else:
        ...
        mnemonic = f"tema({length}, {alpha:.8f}{component_triple_mnemonic(bc, qc, tc)})"
    ...
    length2 = 2 * length - 1
    length3 = 3 * length - 2
    description = f"Triple exponential moving average {mnemonic}"

    return TripleExponentialMovingAverage(length, alpha, length2, length3, first_is_average,
