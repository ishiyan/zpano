...
pub struct FractalAdaptiveSimpleMovingAverage2Params {
    /// The lookback period N for FGDI computation. Must be greater than 1.
    pub period: usize,
...
impl Default for FractalAdaptiveSimpleMovingAverage2Params {
    fn default() -> Self {
        Self {
            period: 30,
            normal_speed: 20,
...
#[repr(u8)]
pub enum FractalAdaptiveSimpleMovingAverage2Output {
    /// The FRASMA2 value.
    Value = 0,
}
...
pub struct FractalAdaptiveSimpleMovingAverage2 {
    line: LineIndicator,
    window: Vec<f64>,
    closes: Vec<f64>,
    period: usize,
...
impl FractalAdaptiveSimpleMovingAverage2 {
    /// Creates a new FractalAdaptiveSimpleMovingAverage2 from the given parameters.
    pub fn new(params: &FractalAdaptiveSimpleMovingAverage2Params) -> Result<Self, String> {
        if params.period < 2 {
            return Err("invalid fractal adaptive simple moving average 2 parameters: period should be greater than 1".to_string());
        }
        if params.normal_speed < 1 {
            return Err("invalid fractal adaptive simple moving average 2 parameters: normal_speed should be greater than 0".to_string());
        }
        ...
        let mnemonic = format!("frasma2({},{}{})", params.period, params.normal_speed, component_triple_mnemonic(bc, qc, tc));
        let description = format!("Fractal adaptive simple moving average 2 {}", mnemonic);
        ...
        Ok(Self {
            ...
            window_count: 0,
            primed: false,
            log_2pm1: (2.0 * (params.period as f64 - 1.0)).ln(),
            ln2: 2.0_f64.ln(),
            inv_p_sq: 1.0 / (period_f * period_f),
        })
    }

    /// Core update logic. Returns the FRASMA2 value or NaN.
    pub fn update(&mut self, sample: f64) -> f64 {
        ...
        let fgdi = 1.0 + (length.ln() + self.ln2) / self.log_2pm1;
...
impl Indicator for FractalAdaptiveSimpleMovingAverage2 {
...
mod tests {
    use super::*;
    use crate::indicators::jean_philippe_poton::fractal_adaptive_simple_moving_average_2::testdata::testdata;

    fn create(period: usize, normal_speed: usize) -> FractalAdaptiveSimpleMovingAverage2 {
        FractalAdaptiveSimpleMovingAverage2::new(&FractalAdaptiveSimpleMovingAverage2Params {
            period,
            normal_speed,
            ..Default::default()
        }).unwrap()
    ...
    #[test]
    fn test_frasma2_nan_passthrough() {
        let mut f = create(5, 20);
        assert!(f.update(f64::NAN).is_nan());
    }

    #[test]
    fn test_frasma2_invalid_period() {
        let result = FractalAdaptiveSimpleMovingAverage2::new(&FractalAdaptiveSimpleMovingAverage2Params {
            period: 1,
            normal_speed: 20,
            ..Default::default()
        });
    ...
    #[test]
    fn test_frasma2_invalid_normal_speed() {
        let result = FractalAdaptiveSimpleMovingAverage2::new(&FractalAdaptiveSimpleMovingAverage2Params {
            period: 5,
            normal_speed: 0,
            ..Default::default()
    ...
    #[test]
    fn test_frasma2_metadata() {
        let f = create(30, 20);
        let meta = f.metadata();
        assert_eq!(meta.identifier, Identifier::FractalAdaptiveSimpleMovingAverage2);
    }
}
