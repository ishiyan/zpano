...
#[derive(Debug)]
pub struct HomodyneDiscriminatorEstimator {
...
impl HomodyneDiscriminatorEstimator {
    pub fn new(p: &CycleEstimatorParams) -> Result<Self, String> {
        ...
        let sl_plus_3ht = sl_plus_3ht_min1 + 1;
        ...
        Ok(Self {
            ...
            alpha_ema_p: alpha_p,
            warm_up_period: p.warm_up_period.max(sl_plus_3ht),
            sl_plus_ht_min1,
            sl_plus_2ht_min2,
            sl_plus_3ht_min3,
            sl_plus_3ht_min2,
...
impl CycleEstimator for HomodyneDiscriminatorEstimator {
    ...
    fn update(&mut self, sample: f64) {
        ...
        if self.is_primed {
            ...
            let re = si * self.smoothed_ip_prev + sq * self.smoothed_q_prev;
            let im = si * self.smoothed_q_prev - sq * self.smoothed_ip_prev;
            self.smoothed_ip_prev = si;
            self.smoothed_q_prev = sq;

            let re = ema(self.alpha_ema_qi, self.one_min_alpha_qi, re, self.re_prev);
            let im = ema(self.alpha_ema_qi, self.one_min_alpha_qi, im, self.im_prev);
            self.re_prev = re;
            self.im_prev = im;
            let period_previous = self.period_val;
            let period_new = TWO_PI / im.atan2(re);
            ...
            if self.sl_plus_3ht_min1 < self.count {
                self.period_val = ema(self.alpha_ema_p, self.one_min_alpha_p, self.period_val, period_previous);
                self.is_primed = true;
            }
        }
...
mod tests {
    use super::*;
    use super::super::testdata_homodyne_discriminator::testdata_homodyne_discriminator::*;

    const EPSILON: f64 = 1e-8;

    fn create_default() -> HomodyneDiscriminatorEstimator {
        ...
        HomodyneDiscriminatorEstimator::new(&p).unwrap()
    }

    fn create_warmup(warm_up: usize) -> HomodyneDiscriminatorEstimator {
        let p = CycleEstimatorParams {
            smoothing_length: 4,
            alpha_ema_quadrature_in_phase: 0.2,
            alpha_ema_period: 0.2,
    ...
    fn test_smoothed() {
        ...
        for i in 0..lprimed {
            hde.update(inp[i]);
            check(i, 0.0, hde.smoothed());
        }
        for i in lprimed..inp.len() {
            hde.update(inp[i]);
            check(i, exp[i], hde.smoothed());
        }
        let previous = hde.smoothed();
        hde.update(f64::NAN);
        check(99999, previous, hde.smoothed());
    }
    ...
    fn test_detrended() {
        ...
        for i in 0..lprimed {
            hde.update(inp[i]);
            check(i, 0.0, hde.detrended());
        }
        for i in lprimed..inp.len() {
            hde.update(inp[i]);
            check(i, exp[i], hde.detrended());
        }
        let previous = hde.detrended();
        hde.update(f64::NAN);
        check(99999, previous, hde.detrended());
    }
    ...
    fn test_quadrature() {
        ...
        for i in 0..lprimed {
            hde.update(inp[i]);
            check(i, 0.0, hde.quadrature());
        }
        for i in lprimed..inp.len() {
            hde.update(inp[i]);
            check(i, exp[i], hde.quadrature());
        }
        let previous = hde.quadrature();
        hde.update(f64::NAN);
        check(99999, previous, hde.quadrature());
    }
    ...
    fn test_in_phase() {
        ...
        for i in 0..lprimed {
            hde.update(inp[i]);
            check(i, 0.0, hde.in_phase());
        }
        for i in lprimed..inp.len() {
            hde.update(inp[i]);
            check(i, exp[i], hde.in_phase());
        }
        let previous = hde.in_phase();
        hde.update(f64::NAN);
        check(99999, previous, hde.in_phase());
    }
    ...
    fn test_period() {
        ...
        for i in 0..lprimed {
            hde.update(inp[i]);
            check(i, not_primed_value, hde.period());
        }
        for i in lprimed..inp.len() {
            hde.update(inp[i]);
            check(i, exp[i], hde.period());
        }
        let previous = hde.period();
        hde.update(f64::NAN);
        check(99999, previous, hde.period());
    }
    ...
    fn test_period_sin() {
        ...
        let mut hde = create_default();
        for i in 0..512 {
            hde.update((omega * i as f64).sin());
        }
        assert!((period - hde.period()).abs() <= 1e-2,
            "period expected {}, actual {}", period, hde.period());
    }
    ...
    fn test_period_sin_min() {
        ...
        assert!((hde.min_period() as f64 - hde.period()).abs() <= 1e-14,
            "min period expected {}, actual {}", hde.min_period(), hde.period());
    }
    ...
    fn test_period_sin_max() {
        ...
        assert!((hde.max_period() as f64 - hde.period()).abs() <= 1e-14,
            "max period expected {}, actual {}", hde.max_period(), hde.period());
    }
    ...
    fn test_primed() {
        ...
        for i in 0..lprimed {
            hde.update(inp[i]);
            assert!(!hde.primed(), "[{}] should not be primed", i + 1);
        }
        for i in lprimed..inp.len() {
            hde.update(inp[i]);
            assert!(hde.primed(), "[{}] should be primed", i + 1);
        }
    }
    ...
    fn test_primed_warmup() {
        let lprimed = 50;
        let mut hde = create_warmup(lprimed);
        let inp = input();
    ...
    fn test_validation_errors() {
        ...
        assert!(HomodyneDiscriminatorEstimator::new(&p).is_ok());
        ...
        assert_eq!(HomodyneDiscriminatorEstimator::new(&p).unwrap_err(), errle);
        ...
        assert_eq!(HomodyneDiscriminatorEstimator::new(&p).unwrap_err(), erraq);
        ...
        assert_eq!(HomodyneDiscriminatorEstimator::new(&p).unwrap_err(), errap);
