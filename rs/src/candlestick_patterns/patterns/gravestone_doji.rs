...
pub fn gravestone_doji(cp: &CandlestickPatterns) -> f64 {
    ...
    let mu_short_ls = cp.mu_less(lower_shadow(b.o, b.l, b.c), &cp.very_short_shadow, 1);
    let mu_long_us = cp.mu_greater(upper_shadow(b.o, b.h, b.c), &cp.very_short_shadow, 1);

    let confidence = fuzzy::t_product_all(&[mu_doji, mu_short_ls, mu_long_us]);
    confidence * 100.0
}
