...
const EVENING_STAR_PENETRATION_FACTOR: f64 = 0.3;
...
pub fn evening_star(cp: &CandlestickPatterns) -> f64 {
    ...
    let threshold = b1.c - rb1 * EVENING_STAR_PENETRATION_FACTOR;
    let width = cp.fuzz_ratio * rb1 * EVENING_STAR_PENETRATION_FACTOR;
    let mu_penetration = cp.mu_lt_raw(b3.c, threshold, width);

    let confidence = fuzzy::t_product_all(&[mu_long1, mu_short2, mu_penetration]);
    -confidence * 100.0
}
