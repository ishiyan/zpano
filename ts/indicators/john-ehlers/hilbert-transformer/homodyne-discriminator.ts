...
export class HilbertTransformerHomodyneDiscriminator implements HilbertTransformerCycleEstimator {
  ...
  public update(sample: number): void {
    ...
    if (this.isPrimed) {
      ...
      const smoothedInPhase = this.emaQuadratureInPhase(this._inPhase[0] - this.jQuadrature[0], this.smoothedInPhasePrevious);
      const smoothedQuadrature = this.emaQuadratureInPhase(this._quadrature[0] + this.jInPhase[0], this.smoothedQuadraturePrevious);
      ...
      const smoothedInPhase = this.emaQuadratureInPhase(this._inPhase[0] - this.jQuadrature[0], this.smoothedInPhasePrevious); // count >= 23
      const smoothedQuadrature = this.emaQuadratureInPhase(this._quadrature[0] + this.jInPhase[0], this.smoothedQuadraturePrevious);
      ...
      if (this.smoothingLengthPlus3HtLengthMin2 === this._count) { // count == 23
        this.rePrevious = re;
        this.imPrevious = im;
        return;
      }
      ...
      this._period = adjustPeriod(this._period, periodPrevious);
      if (this.smoothingLengthPlus3HtLengthMin1 < this._count) { // count > 24
        this._period = this.emaPeriod(this._period, periodPrevious);
        this.isPrimed = true;
      }
    }
