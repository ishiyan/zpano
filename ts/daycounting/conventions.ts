...
export enum DayCountConvention {
    /**
     * Take the difference in seconds between two dates and divides
...
const conventionMap: Record<string, DayCountConvention> = {
...
export function fromString(convention: string): DayCountConvention {
    const normalizedConvention = convention.toLowerCase();
    
    if (!(normalizedConvention in conventionMap)) {
        const validConventions = Object.keys(conventionMap).join(', ');
        throw new Error(
            `Day count convention '${convention}' must be one of: ${validConventions}`
        );
    }
    
    return conventionMap[normalizedConvention];
}
