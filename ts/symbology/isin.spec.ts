...
describe('ISIN validate', () => {
    const cases: [string, boolean][] = [
        ...
        ['AEDFXA14NUL7', true], // Missing AD, AF, AI, AL, AO, AQ, AS, AW.
        ['AEDFXA0M6V00', true],
        ['AGP8696W1045', true],
        ['AG', false],
        ['AMFFCTS10ER7', true],
    ...
    for (const [isin, valid] of cases) {
        it(`validate("${isin}") should be ${valid ? 'valid' : 'invalid'}`, () => {
            if (valid) {
                expect(() => new ISIN(isin).validate()).not.toThrow();
            } else {
                expect(() => new ISIN(isin).validate()).toThrow();
            }
        });
    }
});

describe('ISIN validateCheckDigit', () => {
    const cases: [string, boolean][] = [
        ['@U0000XVGZA3', false],
        ['A@0000XVGZA3', false],
        ['AU@000XVGZA3', false],
    ...
    for (const [isin, valid] of cases) {
        it(`validateCheckDigit("${isin}") should be ${valid ? 'valid' : 'invalid'}`, () => {
            if (valid) {
                expect(() => new ISIN(isin).validateCheckDigit()).not.toThrow();
            } else {
                expect(() => new ISIN(isin).validateCheckDigit()).toThrow();
            }
        });
    }
});

describe('ISIN calculateCheckDigit', () => {
    it('should throw for too-short input', () => {
        expect(() => new ISIN('au0000xvgz').calculateCheckDigit()).toThrow();
    });
});

describe('ISIN validateCountry', () => {
    const cases: [string, boolean][] = [
        ['', false],
        ['Z', false],
        ['ZZ', false],
    ...
    for (const [country, valid] of cases) {
        it(`validateCountry("${country}") should be ${valid}`, () => {
            expect(new ISIN(country).validateCountry()).toBe(valid);
        });
    }
});
