// Package main implements icalc, a command-line indicator calculator.
//
...
func main() {
	if len(os.Args) < 2 {
		fmt.Fprintf(os.Stderr, "usage: icalc <settings.json>\n")
		os.Exit(1)
	}
	...
	fmt.Println()
	fmt.Println("=== Bar Data & Indicator Outputs ===")
	fmt.Println()

	for i, bar := range bars {
		fmt.Printf("Bar[%3d] %s  O=%.4f H=%.4f L=%.4f C=%.4f V=%.0f\n",
		...
		for _, ind := range indicators {
			...
			fmt.Printf("  %-45s primed=%-5v ", meta.Mnemonic, ind.IsPrimed())
			printOutput(meta.Outputs, output)
			fmt.Println()
		}
...
// printMetadata prints metadata for all indicators.
func printMetadata(indicators []core.Indicator) {
	fmt.Println("=== Indicator Metadata ===")
	fmt.Println()
	...
	for i, ind := range indicators {
		...
		for j, out := range meta.Outputs {
			fmt.Printf("    [%d] kind=%d shape=%s mnemonic=%q description=%q\n",
				j, out.Kind, out.Shape, out.Mnemonic, out.Description)
		}

		fmt.Printf("  Full metadata JSON:\n  %s\n\n", metaJSON)
	}
}

// printOutput prints output values inline.
func printOutput(outputsMeta []outputs.Metadata, output core.Output) {
	for i, val := range output {
		name := fmt.Sprintf("out[%d]", i)
		if i < len(outputsMeta) {
			name = outputsMeta[i].Mnemonic
		}

		switch v := val.(type) {
		case entities.Scalar:
			if math.IsNaN(v.Value) {
				fmt.Printf("%s=NaN ", name)
			} else {
				fmt.Printf("%s=%.4f ", name, v.Value)
			}
		case *outputs.Band:
			if v.IsEmpty() {
				fmt.Printf("%s=Band(NaN) ", name)
			} else {
...
func testBars() []entities.Bar {
	...
	// Open data is not available in the standard TA-Lib test set used in this
	// codebase, so we approximate open as the previous close (or the first close).
