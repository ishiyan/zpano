...
func TestCandlestickMomentumIndexData(t *testing.T) {
	t.Parallel()

	for _, combo := range cmiCombos() {
		combo := combo
		t.Run(combo.name, func(t *testing.T) {
			t.Parallel()

			cmi, err := NewCandlestickMomentumIndex(&Params{
			...
			for i := 0; i < len(testInput); i++ {
				cmiVal, signalVal := cmi.Update(testOpen[i], testInput[i])

				checkVal(t, "cmi", i, combo.expCmi[i], cmiVal)
...
func TestCandlestickMomentumIndexPassthrough(t *testing.T) {
	t.Parallel()

	// All EMA stages and the signal EMA are passthroughs, so the oscillator is
	// sign(close-open)*100 and the signal equals it.
	cmi, err := NewCandlestickMomentumIndex(&Params{R: 1, S: 1, U: 1, Ul: 1})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	tests := []struct {
		...
		{"up candle", 10.0, 12.0, 100.0},
		{"down candle", 12.0, 11.0, -100.0},
		{"doji", 11.0, 11.0, 0.0},
	}

	for i, tt := range tests {
		cmiVal, signalVal := cmi.Update(tt.open, tt.close)

		checkVal(t, tt.name+" cmi", i, tt.exp, cmiVal)
		checkVal(t, tt.name+" signal", i, tt.exp, signalVal)
	}
}

func TestCandlestickMomentumIndexIsPrimed(t *testing.T) {
	t.Parallel()

	// The candle momentum is defined from bar 0, so there is no NaN warm-up.
	cmi, err := NewCandlestickMomentumIndex(DefaultParams())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	cmi.Update(testOpen[0], testInput[0])
...
func TestCandlestickMomentumIndexUpdateBar(t *testing.T) {
	t.Parallel()

	cmi, err := NewCandlestickMomentumIndex(&Params{R: 20, S: 5, U: 3, Ul: ul})
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	for i := 0; i < len(testInput); i++ {
		output = cmi.UpdateBar(&entities.Bar{
	...
	checkVal(t, "cmi", last, expectedR20_S5_U3[last], output[0].(entities.Scalar).Value)
	checkVal(t, "signal", last, expectedR20_S5_U3_SIG_UL3[last], output[1].(entities.Scalar).Value)
}

func TestCandlestickMomentumIndexUpdateEntities(t *testing.T) {
	t.Parallel()

	tm := time.Now()

	t.Run("quote maps bid to open and ask to close", func(t *testing.T) {
		t.Parallel()
		...
		output := cmi.UpdateQuote(&entities.Quote{Time: tm, Bid: 10.0, Ask: 12.0})

		checkVal(t, "cmi", 0, 100.0, output[0].(entities.Scalar).Value)
		checkVal(t, "signal", 0, 100.0, output[1].(entities.Scalar).Value)
	})

	t.Run("scalar has a zero candle body", func(t *testing.T) {
		t.Parallel()
		...
		output := cmi.UpdateScalar(&entities.Scalar{Time: tm, Value: 10.0})

		checkVal(t, "cmi", 0, 0.0, output[0].(entities.Scalar).Value)
		checkVal(t, "signal", 0, 0.0, output[1].(entities.Scalar).Value)
	})

	t.Run("trade has a zero candle body", func(t *testing.T) {
		t.Parallel()
		...
		output := cmi.UpdateTrade(&entities.Trade{Time: tm, Price: 10.0, Volume: 1.0})
...
func TestCandlestickMomentumIndexMnemonic(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{"all parameters zero", &Params{}, "cmi(20,5,3,3)"},
		{"default parameters", DefaultParams(), "cmi(20,5,3,3)"},
		{"double smoothing", &Params{R: 25, S: 13, U: 1, Ul: 3}, "cmi(25,13,1,3)"},
		{"passthrough signal", &Params{R: 20, S: 5, U: 3, Ul: 1}, "cmi(20,5,3,1)"},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			cmi, err := NewCandlestickMomentumIndex(tt.params)
			if err != nil {
				t.Fatalf("unexpected error: %v", err)
			}

			if cmi.mnemonic != tt.exp {
				t.Errorf("mnemonic: expected '%s', got '%s'", tt.exp, cmi.mnemonic)
			}

			expDesc := "Candlestick Momentum Index " + tt.exp
			if meta := cmi.Metadata(); meta.Description != expDesc {
...
func TestCandlestickMomentumIndexMetadata(t *testing.T) {
	t.Parallel()
	...
	if meta.Identifier != core.CandlestickMomentumIndex {
		t.Errorf("identifier: expected CandlestickMomentumIndex, got %v", meta.Identifier)
	}

	if meta.Mnemonic != "cmi(20,5,3,3)" {
		t.Errorf("mnemonic: expected 'cmi(20,5,3,3)', got '%s'", meta.Mnemonic)
	}
...
func TestCandlestickMomentumIndexInvalidParams(t *testing.T) {
	t.Parallel()
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err := NewCandlestickMomentumIndex(tt.params); err == nil {
