...
func testCreate(period, normalSpeed int) *FractalAdaptiveSimpleMovingAverage {
	f, _ := NewFractalAdaptiveSimpleMovingAverage(&Params{Period: period, NormalSpeed: normalSpeed})
	return f
}

func TestFractalAdaptiveSimpleMovingAverageUpdate(t *testing.T) {
	t.Parallel()
...
func TestFractalAdaptiveSimpleMovingAverageIsPrimed(t *testing.T) {
	t.Parallel()
...
func TestFractalAdaptiveSimpleMovingAverageNaN(t *testing.T) {
	t.Parallel()
...
func TestFractalAdaptiveSimpleMovingAverageInvalidParams(t *testing.T) {
	t.Parallel()

	_, err := NewFractalAdaptiveSimpleMovingAverage(&Params{Period: 1, NormalSpeed: 20})
	if err == nil {
		t.Error("expected error for period=1")
	}

	_, err = NewFractalAdaptiveSimpleMovingAverage(&Params{Period: 5, NormalSpeed: 0})
	if err == nil {
		t.Error("expected error for normal_speed=0")
	}
}

func TestFractalAdaptiveSimpleMovingAverageMetadata(t *testing.T) {
	t.Parallel()

	f := testCreate(30, 20)
	meta := f.Metadata()

	if meta.Identifier != core.FractalAdaptiveSimpleMovingAverage {
		t.Errorf("expected identifier %v, got %v", core.FractalAdaptiveSimpleMovingAverage, meta.Identifier)
	}
}
