...
func New(identifier core.Identifier, params string) (core.Indicator, error) {
	...
	case core.MeanDeviationIndex:
		p := meandeviationindex.DefaultParams()
		if err := unmarshal(b, p); err != nil {
			return nil, err
		}
	...
	case core.DoubleSmoothedMomenta:
		p := doublesmoothedmomenta.DefaultParams()
		if err := unmarshal(b, p); err != nil {
			return nil, err
		}
	...
	case core.CandlestickStrengthIndex:
		p := candlestickstrengthindex.DefaultParams()
		if err := unmarshal(b, p); err != nil {
			return nil, err
		}

		return candlestickstrengthindex.NewCandlestickStrengthIndex(p)

	case core.StochasticMomentumIndex:
		p := stochasticmomentumindex.DefaultParams()
		if err := unmarshal(b, p); err != nil {
			return nil, err
		}
