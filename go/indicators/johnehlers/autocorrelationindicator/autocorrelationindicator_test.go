...
func testAciTime() time.Time {
	return time.Date(2021, time.April, 1, 0, 0, 0, 0, time.UTC)
}

// testAciInput is the 252-entry TA-Lib MAMA reference series (shared with CBPS/DFTS tests).
//
//nolint:dupl
func testAciInput() []float64 {
	return []float64{
...
const (
	testAciTolerance = 1e-12
	testAciMinMaxTol = 1e-10
)
...
// Snapshots captured from the Go implementation. The correlation math is
// additionally sanity-checked in TestAutoCorrelationIndicatorSyntheticSine below.
//
