...
// test_7-1: 1-pole HP, no zero-mean (shortest=10, longest=48).
// test_7-2: 1-pole HP, zero-mean (shortest=10, longest=48), column Filt2.
// test_7-3: 2-pole HP (shortest=40, longest=80), column Filt.
...
// Expected output for test 7-1: 1-pole HP, no zero-mean (HP_LP_Roofing_Filter column).
func testExpected71() []float64 {
	return []float64{
...
// Expected output for test 7-2: 1-pole HP, zero-mean (shortest=10, longest=48), column Filt2.
func testExpected72() []float64 {
	return []float64{
...
// Expected output for test 7-3: 2-pole HP (shortest=40, longest=80), column Filt.
func testExpected73() []float64 {
	return []float64{
...
func TestRoofingFilterMetadata(t *testing.T) {
	...
	check("Type", core.RoofingFilter, act.Type)
	check("len(Outputs)", 1, len(act.Outputs))
	check("Outputs[0].Kind", int(RoofingFilterValue), act.Outputs[0].Kind)
