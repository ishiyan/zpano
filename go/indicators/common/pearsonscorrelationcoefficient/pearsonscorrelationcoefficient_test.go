...
//nolint:lll
// Input data: high and low arrays from TA-Lib test_data.c (252 entries each).
//
// TaLib CORREL test spot-checks (from test_per_hl.c):
...
//
// Excel verification data: excel-period-20-CORREL.csv (252 rows, first 19 are NaN).
...
func testExcelExpected() []float64 {
	return []float64{
...
func TestPearsonsCorrelationCoefficientMetadata(t *testing.T) {
	...
	check("Type", core.PearsonsCorrelationCoefficient, act.Type)
