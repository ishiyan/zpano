...
use super::json::{has_key, is_empty_object, get_f64, get_usize, get_i32, get_i64, get_bool, JsonValue};
...
use crate::indicators::common::weighted_moving_average::{WeightedMovingAverage, WeightedMovingAverageParams};
use crate::indicators::common::triangular_moving_average::{TriangularMovingAverage, TriangularMovingAverageParams};
use crate::indicators::common::exponential_moving_average::{ExponentialMovingAverage, ExponentialMovingAverageLengthParams, ExponentialMovingAverageSmoothingFactorParams};
...
use crate::indicators::common::rate_of_change_percent::{RateOfChangePercent, RateOfChangePercentParams};
use crate::indicators::common::rate_of_change_ratio::{RateOfChangeRatio, RateOfChangeRatioParams};
use crate::indicators::common::absolute_price_oscillator::{AbsolutePriceOscillator, AbsolutePriceOscillatorParams};
use crate::indicators::common::pearsons_correlation_coefficient::{PearsonsCorrelationCoefficient, PearsonsCorrelationCoefficientParams};
...
use crate::indicators::custom::maximum_entropy_spectrum::maximum_entropy_spectrum::{MaximumEntropySpectrum, MaximumEntropySpectrumParams};

// ── donald lambert ───────────────────────────────────────────────────────────
use crate::indicators::donald_lambert::commodity_channel_index::{CommodityChannelIndex, CommodityChannelIndexParams};
...
// ── jack hutson ──────────────────────────────────────────────────────────────
use crate::indicators::jack_hutson::triple_exponential_moving_average_oscillator::{TripleExponentialMovingAverageOscillator, TripleExponentialMovingAverageOscillatorParams};

// ── john bollinger ───────────────────────────────────────────────────────────
use crate::indicators::john_bollinger::bollinger_bands::{BollingerBands, BollingerBandsParams};
use crate::indicators::john_bollinger::bollinger_bands_trend::{BollingerBandsTrend, BollingerBandsTrendParams};
...
// ── joseph granville ─────────────────────────────────────────────────────────
use crate::indicators::joseph_granville::on_balance_volume::{OnBalanceVolume, OnBalanceVolumeParams};

// ── larry williams ───────────────────────────────────────────────────────────
use crate::indicators::larry_williams::williams_percent_r::{WilliamsPercentR, WilliamsPercentRParams};
use crate::indicators::larry_williams::ultimate_oscillator::{UltimateOscillator, UltimateOscillatorParams};

// ── marc chaikin ─────────────────────────────────────────────────────────────
use crate::indicators::marc_chaikin::advance_decline::{AdvanceDecline, AdvanceDeclineParams};
use crate::indicators::marc_chaikin::advance_decline_oscillator::{AdvanceDeclineOscillator, AdvanceDeclineOscillatorParams};

// ── mark jurik ───────────────────────────────────────────────────────────────
use crate::indicators::mark_jurik::jurik_moving_average::{JurikMovingAverage, JurikMovingAverageParams};

// ── patrick mulloy ───────────────────────────────────────────────────────────
use crate::indicators::patrick_mulloy::double_exponential_moving_average::{DoubleExponentialMovingAverage, DoubleExponentialMovingAverageLengthParams, DoubleExponentialMovingAverageSmoothingFactorParams};
use crate::indicators::patrick_mulloy::triple_exponential_moving_average::{TripleExponentialMovingAverage, TripleExponentialMovingAverageLengthParams, TripleExponentialMovingAverageSmoothingFactorParams};

// ── perry kaufman ────────────────────────────────────────────────────────────
use crate::indicators::perry_kaufman::kaufman_adaptive_moving_average::{KaufmanAdaptiveMovingAverage, KaufmanAdaptiveMovingAverageLengthParams, KaufmanAdaptiveMovingAverageSmoothingFactorParams};
...
// ── tushar chande ────────────────────────────────────────────────────────────
use crate::indicators::tushar_chande::chande_momentum_oscillator::{ChandeMomentumOscillator, ChandeMomentumOscillatorParams};
use crate::indicators::tushar_chande::stochastic_relative_strength_index::{StochasticRelativeStrengthIndex, StochasticRelativeStrengthIndexParams};
...
use crate::indicators::welles_wilder::average_true_range::{AverageTrueRange, AverageTrueRangeParams};
use crate::indicators::welles_wilder::normalized_average_true_range::{NormalizedAverageTrueRange, NormalizedAverageTrueRangeParams};
use crate::indicators::welles_wilder::directional_movement_minus::{DirectionalMovementMinus, DirectionalMovementMinusParams};
use crate::indicators::welles_wilder::directional_movement_plus::{DirectionalMovementPlus, DirectionalMovementPlusParams};
use crate::indicators::welles_wilder::directional_indicator_minus::{DirectionalIndicatorMinus, DirectionalIndicatorMinusParams};
use crate::indicators::welles_wilder::directional_indicator_plus::{DirectionalIndicatorPlus, DirectionalIndicatorPlusParams};
use crate::indicators::welles_wilder::directional_movement_index::{DirectionalMovementIndex, DirectionalMovementIndexParams};
use crate::indicators::welles_wilder::average_directional_movement_index::{AverageDirectionalMovementIndex, AverageDirectionalMovementIndexParams};
use crate::indicators::welles_wilder::average_directional_movement_index_rating::{AverageDirectionalMovementIndexRating, AverageDirectionalMovementIndexRatingParams};
use crate::indicators::welles_wilder::relative_strength_index::{RelativeStrengthIndex, RelativeStrengthIndexParams};
use crate::indicators::welles_wilder::parabolic_stop_and_reverse::{ParabolicStopAndReverse, ParabolicStopAndReverseParams};
...
pub fn create_indicator(identifier: Identifier, params_json: &str) -> Result<Box<dyn Indicator>, String> {
    ...
    match identifier {
        ...
        Identifier::SimpleMovingAverage => {
            ...
            if let Some(v) = get_usize(&params, "length") { p.length = v; }
        ...
        Identifier::ExponentialMovingAverage => {
            if has_key(&params, "smoothingFactor") {
                ...
                if let Some(v) = get_f64(&params, "smoothingFactor") { p.smoothing_factor = v; }
                if let Some(v) = get_bool(&params, "firstIsAverage") { p.first_is_average = v; }
                Ok(Box::new(ExponentialMovingAverage::new_from_smoothing_factor(&p)?))
            } else {
                let mut p = ExponentialMovingAverageLengthParams::default();
                if let Some(v) = get_i64(&params, "length") { p.length = v; }
                if let Some(v) = get_bool(&params, "firstIsAverage") { p.first_is_average = v; }
        ...
        Identifier::AbsolutePriceOscillator => {
            ...
            if let Some(v) = get_i64(&params, "fastLength") { p.fast_length = v; }
            if let Some(v) = get_i64(&params, "slowLength") { p.slow_length = v; }
            if let Some(v) = get_bool(&params, "firstIsAverage") { p.first_is_average = v; }
        ...
        Identifier::GoertzelSpectrum => {
            ...
            if !is_empty_object(&params) {
                ...
                if let Some(v) = get_usize(&params, "spectrumResolution") { p.spectrum_resolution = v; }
                if let Some(v) = get_bool(&params, "isFirstOrder") { p.is_first_order = v; }
                if let Some(v) = get_bool(&params, "disableSpectralDilationCompensation") { p.disable_spectral_dilation_compensation = v; }
                if let Some(v) = get_bool(&params, "disableAutomaticGainControl") { p.disable_automatic_gain_control = v; }
                if let Some(v) = get_f64(&params, "automaticGainControlDecayFactor") { p.automatic_gain_control_decay_factor = v; }
                if let Some(v) = get_bool(&params, "fixedNormalization") { p.fixed_normalization = v; }
            }
        ...
        Identifier::Stochastic => {
            ...
            if let Some(v) = get_usize(&params, "fastKLength") { p.fast_k_length = v; }
            if let Some(v) = get_usize(&params, "slowKLength") { p.slow_k_length = v; }
            if let Some(v) = get_usize(&params, "slowDLength") { p.slow_d_length = v; }
        ...
        Identifier::PercentagePriceOscillator => {
            ...
            if let Some(v) = get_usize(&params, "fastLength") { p.fast_length = v; }
            if let Some(v) = get_usize(&params, "slowLength") { p.slow_length = v; }
        ...
        Identifier::MovingAverageConvergenceDivergence => {
            ...
            if let Some(v) = get_usize(&params, "fastLength") { p.fast_length = v; }
            if let Some(v) = get_usize(&params, "slowLength") { p.slow_length = v; }
            if let Some(v) = get_usize(&params, "signalLength") { p.signal_length = v; }
        ...
        Identifier::BalanceOfPower => {
            Ok(Box::new(BalanceOfPower::new(&BalanceOfPowerParams)?))
        }
        ...
        Identifier::BollingerBands => {
            ...
            if let Some(v) = get_usize(&params, "length") { p.length = v; }
            if let Some(v) = get_f64(&params, "upperMultiplier") { p.upper_multiplier = v; }
            if let Some(v) = get_f64(&params, "lowerMultiplier") { p.lower_multiplier = v; }
        ...
        Identifier::SuperSmoother => {
            ...
            if let Some(v) = get_i64(&params, "shortestCyclePeriod") { p.shortest_cycle_period = v; }
        ...
        Identifier::CyberCycle => {
            if has_key(&params, "smoothingFactor") {
                ...
                if let Some(v) = get_f64(&params, "smoothingFactor") { p.smoothing_factor = v; }
                if let Some(v) = get_i64(&params, "signalLag") { p.signal_lag = v; }
        ...
        Identifier::InstantaneousTrendLine => {
            if has_key(&params, "smoothingFactor") {
                ...
                if let Some(v) = get_i32(&params, "length") { p.length = v; }
        ...
        Identifier::ZeroLagExponentialMovingAverage => {
            ...
            if let Some(v) = get_f64(&params, "smoothingFactor") { p.smoothing_factor = v; }
            if let Some(v) = get_f64(&params, "velocityGainFactor") { p.velocity_gain_factor = v; }
            if let Some(v) = get_i32(&params, "velocityMomentumLength") { p.velocity_momentum_length = v; }
        ...
        Identifier::ZeroLagErrorCorrectingExponentialMovingAverage => {
            ...
            if let Some(v) = get_f64(&params, "smoothingFactor") { p.smoothing_factor = v; }
            if let Some(v) = get_f64(&params, "gainLimit") { p.gain_limit = v; }
            if let Some(v) = get_f64(&params, "gainStep") { p.gain_step = v; }
            Ok(Box::new(ZeroLagErrorCorrectingExponentialMovingAverage::new(&p)?))
        }

        Identifier::RoofingFilter => {
            let mut p = RoofingFilterParams::default();
            if let Some(v) = get_usize(&params, "shortestCyclePeriod") { p.shortest_cycle_period = v; }
            if let Some(v) = get_usize(&params, "longestCyclePeriod") { p.longest_cycle_period = v; }
            Ok(Box::new(RoofingFilter::new(&p)?))
        }

        Identifier::MesaAdaptiveMovingAverage => {
            if has_key(&params, "fastLimitSmoothingFactor") || has_key(&params, "slowLimitSmoothingFactor") {
                ...
                Ok(Box::new(MesaAdaptiveMovingAverage::new_smoothing_factor(&p)?))
        ...
        Identifier::DominantCycle => {
            if is_empty_object(&params) {
                ...
                if let Some(v) = get_f64(&params, "alphaEmaPeriodAdditional") { p.alpha_ema_period_additional = v; }
        ...
        Identifier::HilbertTransformerInstantaneousTrendLine => {
            if is_empty_object(&params) {
                Ok(Box::new(HilbertTransformerInstantaneousTrendLine::new_default()?))
            } else {
                let mut p = HilbertTransformerInstantaneousTrendLineParams::default();
                if let Some(v) = get_f64(&params, "alphaEmaPeriodAdditional") { p.alpha_ema_period_additional = v; }
                if let Some(v) = get_usize(&params, "trendLineSmoothingLength") { p.trend_line_smoothing_length = v; }
                if let Some(v) = get_f64(&params, "cyclePartMultiplier") { p.cycle_part_multiplier = v; }
        ...
        Identifier::TrendCycleMode => {
            if is_empty_object(&params) {
                ...
                if let Some(v) = get_usize(&params, "trendLineSmoothingLength") { p.trend_line_smoothing_length = v; }
                if let Some(v) = get_f64(&params, "cyclePartMultiplier") { p.cycle_part_multiplier = v; }
                if let Some(v) = get_f64(&params, "separationPercentage") { p.separation_percentage = v; }
        ...
        Identifier::CoronaSpectrum => {
            ...
            if !is_empty_object(&params) {
                if let Some(v) = get_f64(&params, "minRasterValue") { p.min_raster_value = v; }
                if let Some(v) = get_f64(&params, "maxRasterValue") { p.max_raster_value = v; }
                if let Some(v) = get_f64(&params, "minParameterValue") { p.min_parameter_value = v; }
                if let Some(v) = get_f64(&params, "maxParameterValue") { p.max_parameter_value = v; }
                if let Some(v) = get_i32(&params, "highPassFilterCutoff") { p.high_pass_filter_cutoff = v; }
            }
        ...
        Identifier::CoronaSignalToNoiseRatio => {
            ...
            if !is_empty_object(&params) {
                if let Some(v) = get_i32(&params, "rasterLength") { p.raster_length = v; }
                if let Some(v) = get_f64(&params, "maxRasterValue") { p.max_raster_value = v; }
                if let Some(v) = get_f64(&params, "minParameterValue") { p.min_parameter_value = v; }
                if let Some(v) = get_f64(&params, "maxParameterValue") { p.max_parameter_value = v; }
                if let Some(v) = get_i32(&params, "highPassFilterCutoff") { p.high_pass_filter_cutoff = v; }
                if let Some(v) = get_i32(&params, "minimalPeriod") { p.minimal_period = v; }
                if let Some(v) = get_i32(&params, "maximalPeriod") { p.maximal_period = v; }
            }
        ...
        Identifier::AutoCorrelationIndicator => {
            ...
            if !is_empty_object(&params) {
                if let Some(v) = get_i32(&params, "minLag") { p.min_lag = v; }
                if let Some(v) = get_i32(&params, "maxLag") { p.max_lag = v; }
                if let Some(v) = get_i32(&params, "smoothingPeriod") { p.smoothing_period = v; }
                if let Some(v) = get_i32(&params, "averagingLength") { p.averaging_length = v; }
            }
        ...
        Identifier::AutoCorrelationPeriodogram => {
            ...
            if !is_empty_object(&params) {
                if let Some(v) = get_i32(&params, "minPeriod") { p.min_period = v; }
                if let Some(v) = get_i32(&params, "maxPeriod") { p.max_period = v; }
                if let Some(v) = get_i32(&params, "averagingLength") { p.averaging_length = v; }
                if let Some(v) = get_bool(&params, "disableAutomaticGainControl") { p.disable_automatic_gain_control = v; }
                if let Some(v) = get_f64(&params, "automaticGainControlDecayFactor") { p.automatic_gain_control_decay_factor = v; }
                if let Some(v) = get_bool(&params, "fixedNormalization") { p.fixed_normalization = v; }
        ...
        Identifier::UltimateOscillator => {
            ...
            if let Some(v) = get_usize(&params, "length1") { p.length1 = v; }
            if let Some(v) = get_usize(&params, "length2") { p.length2 = v; }
            if let Some(v) = get_usize(&params, "length3") { p.length3 = v; }
        ...
        Identifier::AdvanceDecline => {
            Ok(Box::new(AdvanceDecline::new(&AdvanceDeclineParams)?))
        }
        ...
        Identifier::JurikMovingAverage => {
            ...
            if let Some(v) = get_usize(&params, "length") { p.length = v; }
            if let Some(v) = get_i32(&params, "phase") { p.phase = v; }
        ...
        Identifier::DoubleExponentialMovingAverage => {
            if has_key(&params, "smoothingFactor") {
                ...
                if let Some(v) = get_f64(&params, "smoothingFactor") { p.smoothing_factor = v; }
                if let Some(v) = get_bool(&params, "firstIsAverage") { p.first_is_average = v; }
                Ok(Box::new(DoubleExponentialMovingAverage::new_from_smoothing_factor(&p)?))
            } else {
                let mut p = DoubleExponentialMovingAverageLengthParams::default();
                if let Some(v) = get_i64(&params, "length") { p.length = v; }
                if let Some(v) = get_bool(&params, "firstIsAverage") { p.first_is_average = v; }
                Ok(Box::new(DoubleExponentialMovingAverage::new_from_length(&p)?))
            }
        }
        ...
        Identifier::TripleExponentialMovingAverage => {
            if has_key(&params, "smoothingFactor") {
                ...
                if let Some(v) = get_f64(&params, "smoothingFactor") { p.smoothing_factor = v; }
                if let Some(v) = get_bool(&params, "firstIsAverage") { p.first_is_average = v; }
                Ok(Box::new(TripleExponentialMovingAverage::new_from_smoothing_factor(&p)?))
            } else {
                let mut p = TripleExponentialMovingAverageLengthParams::default();
                if let Some(v) = get_i64(&params, "length") { p.length = v; }
                if let Some(v) = get_bool(&params, "firstIsAverage") { p.first_is_average = v; }
                Ok(Box::new(TripleExponentialMovingAverage::new_from_length(&p)?))
            }
        }
        ...
        Identifier::KaufmanAdaptiveMovingAverage => {
            if has_key(&params, "fastestSmoothingFactor") || has_key(&params, "slowestSmoothingFactor") {
                let mut p = KaufmanAdaptiveMovingAverageSmoothingFactorParams::default();
                if let Some(v) = get_usize(&params, "efficiencyRatioLength") { p.efficiency_ratio_length = v; }
                if let Some(v) = get_f64(&params, "fastestSmoothingFactor") { p.fastest_smoothing_factor = v; }
                if let Some(v) = get_f64(&params, "slowestSmoothingFactor") { p.slowest_smoothing_factor = v; }
                Ok(Box::new(KaufmanAdaptiveMovingAverage::new_from_smoothing_factors(&p)?))
            } else {
                let mut p = KaufmanAdaptiveMovingAverageLengthParams::default();
                if let Some(v) = get_usize(&params, "efficiencyRatioLength") { p.efficiency_ratio_length = v; }
                if let Some(v) = get_usize(&params, "fastestLength") { p.fastest_length = v; }
                if let Some(v) = get_usize(&params, "slowestLength") { p.slowest_length = v; }
                Ok(Box::new(KaufmanAdaptiveMovingAverage::new_from_lengths(&p)?))
            }
        }
        ...
        Identifier::T2ExponentialMovingAverage => {
            if has_key(&params, "smoothingFactor") {
                ...
                if let Some(v) = get_f64(&params, "smoothingFactor") { p.smoothing_factor = v; }
                if let Some(v) = get_f64(&params, "volumeFactor") { p.volume_factor = v; }
                if let Some(v) = get_bool(&params, "firstIsAverage") { p.first_is_average = v; }
                Ok(Box::new(T2ExponentialMovingAverage::new_from_smoothing_factor(&p)?))
            } else {
        ...
        Identifier::T3ExponentialMovingAverage => {
            if has_key(&params, "smoothingFactor") {
                ...
                if let Some(v) = get_f64(&params, "volumeFactor") { p.volume_factor = v; }
                if let Some(v) = get_bool(&params, "firstIsAverage") { p.first_is_average = v; }
                Ok(Box::new(T3ExponentialMovingAverage::new_from_smoothing_factor(&p)?))
            } else {
        ...
        Identifier::StochasticRelativeStrengthIndex => {
            ...
            if let Some(v) = get_usize(&params, "length") { p.length = v; }
            if let Some(v) = get_usize(&params, "fastKLength") { p.fast_k_length = v; }
            if let Some(v) = get_usize(&params, "fastDLength") { p.fast_d_length = v; }
        ...
        Identifier::TrueRange => {
            Ok(Box::new(TrueRange::new(&TrueRangeParams)?))
        }
...
mod tests {
    ...
    fn test_create_ema_length() {
        let ind = create_indicator(Identifier::ExponentialMovingAverage, r#"{"length": 10}"#).unwrap();
        assert!(!ind.is_primed());
    }

    #[test]
    fn test_create_ema_smoothing_factor() {
        let ind = create_indicator(Identifier::ExponentialMovingAverage, r#"{"smoothingFactor": 0.1}"#).unwrap();
        assert!(!ind.is_primed());
    }
    ...
    fn test_create_default_indicators() {
        ...
        for id in ids {
            ...
            assert!(result.is_ok(), "failed to create {:?}: {:?}", id, result.err());
        }
    }
