...
/// Enumerates the outputs of the fractal adaptive simple moving average 2 indicator.
pub const FractalAdaptiveSimpleMovingAverage2Output = enum(u8) {
    /// The scalar value of the FRASMA2.
    value = 0,
};

/// Parameters to create an instance of the fractal adaptive simple moving average 2 indicator.
pub const FractalAdaptiveSimpleMovingAverage2Params = struct {
    /// The lookback period N for FGDI computation. Must be > 1.
    period: usize,
...
/// path segments. The indicator is not primed during the first `period` updates.
pub const FractalAdaptiveSimpleMovingAverage2 = struct {
    line: LineIndicator,
    window: []f64,
    ...
    pub fn init(allocator: std.mem.Allocator, params: FractalAdaptiveSimpleMovingAverage2Params) !FractalAdaptiveSimpleMovingAverage2 {
        if (params.period < 2) {
        ...
        const mnemonic_slice = std.fmt.bufPrint(&mnemonic_buf, "frasma2({d},{d}{s})", .{ params.period, params.normal_speed, triple }) catch
            return error.MnemonicTooLong;
        const mnemonic_len = mnemonic_slice.len;

        var description_buf: [128]u8 = undefined;
        const desc_slice = std.fmt.bufPrint(&description_buf, "Fractal adaptive simple moving average 2 {s}", .{mnemonic_slice}) catch
            return error.MnemonicTooLong;
        ...
        const period_m1_f: f64 = @floatFromInt(params.period - 1);
    ...
    pub fn deinit(self: *FractalAdaptiveSimpleMovingAverage2) void {
        self.allocator.free(self.window);
        self.closes.deinit(self.allocator);
    }

    pub fn fixSlices(self: *FractalAdaptiveSimpleMovingAverage2) void {
        self.line.mnemonic = self.mnemonic_buf[0..self.mnemonic_len];
        self.line.description = self.description_buf[0..self.description_len];
    }

    pub fn update(self: *FractalAdaptiveSimpleMovingAverage2, sample: f64) f64 {
        ...
        const fgdi = 1.0 + (@log(length) + self.ln2) / self.log_2pm1;
    ...
    pub fn isPrimed(self: *const FractalAdaptiveSimpleMovingAverage2) bool {
        return self.primed;
    }

    pub fn getMetadata(self: *const FractalAdaptiveSimpleMovingAverage2, out: *Metadata) void {
        build_metadata_mod.buildMetadata(
            out,
            .fractal_adaptive_simple_moving_average_2,
            self.line.mnemonic,
    ...
    pub fn updateScalar(self: *FractalAdaptiveSimpleMovingAverage2, sample: *const Scalar) OutputArray {
        const value = self.update(sample.value);
        return LineIndicator.wrapScalar(sample.time, value);
    }

    pub fn updateBar(self: *FractalAdaptiveSimpleMovingAverage2, sample: *const Bar) OutputArray {
        const value = self.update(self.line.extractBar(sample));
        return LineIndicator.wrapScalar(sample.time, value);
    }

    pub fn updateQuote(self: *FractalAdaptiveSimpleMovingAverage2, sample: *const Quote) OutputArray {
        const value = self.update(self.line.extractQuote(sample));
        return LineIndicator.wrapScalar(sample.time, value);
    }

    pub fn updateTrade(self: *FractalAdaptiveSimpleMovingAverage2, sample: *const Trade) OutputArray {
        const value = self.update(self.line.extractTrade(sample));
        return LineIndicator.wrapScalar(sample.time, value);
    }

    pub fn indicator(self: *FractalAdaptiveSimpleMovingAverage2) indicator_mod.Indicator {
        return .{
            .ptr = @ptrCast(self),
            .vtable = &vtable,
        };
    ...
    fn vtableIsPrimed(ptr: *anyopaque) bool {
        const self: *FractalAdaptiveSimpleMovingAverage2 = @ptrCast(@alignCast(ptr));
        return self.isPrimed();
    }

    fn vtableMetadata(ptr: *anyopaque, out: *Metadata) void {
        const self: *const FractalAdaptiveSimpleMovingAverage2 = @ptrCast(@alignCast(ptr));
...
test "frasma2 update period 120" {
    const input = testdata.testInput();
...
test "frasma2 invalid period" {
    const result = FractalAdaptiveSimpleMovingAverage2.init(testing.allocator, .{ .period = 1, .normal_speed = 20 });
    try testing.expectError(error.InvalidPeriod, result);
}

test "frasma2 invalid normal_speed" {
    const result = FractalAdaptiveSimpleMovingAverage2.init(testing.allocator, .{ .period = 5, .normal_speed = 0 });
