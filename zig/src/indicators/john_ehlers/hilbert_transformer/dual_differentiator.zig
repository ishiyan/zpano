...
/// Hilbert transformer cycle estimator using the Dual Differentiator technique.
///
/// John Ehlers, Rocket Science for Traders, Wiley, 2001, 0471405671, pp 70-74.
pub const DualDifferentiatorEstimator = struct {
    ...
    pub fn init(p: *const ht.CycleEstimatorParams) ht.VerifyError!DualDifferentiatorEstimator {
        ...
        return .{
            ...
            .warm_up_period = @max(p.warm_up_period, sl_3ht_1),
    ...
    pub fn smoothingLength(self: *const DualDifferentiatorEstimator) usize {
        return self.smoothing_length;
    }

    pub fn minPeriod(self: *const DualDifferentiatorEstimator) usize {
        return self.min_period;
    }

    pub fn maxPeriod(self: *const DualDifferentiatorEstimator) usize {
        return self.max_period;
    }

    pub fn warmUpPeriod(self: *const DualDifferentiatorEstimator) usize {
        return self.warm_up_period;
    }

    pub fn alphaEmaQuadratureInPhase(self: *const DualDifferentiatorEstimator) f64 {
        return self.alpha_ema_quadrature_in_phase;
    }

    pub fn alphaEmaPeriod(self: *const DualDifferentiatorEstimator) f64 {
        return self.alpha_ema_period;
    }

    pub fn count(self: *const DualDifferentiatorEstimator) usize {
        return self._count;
    }

    pub fn primed(self: *const DualDifferentiatorEstimator) bool {
        return self.is_warmed_up;
    }

    pub fn period(self: *const DualDifferentiatorEstimator) f64 {
        return self._period;
    }

    pub fn inPhase(self: *const DualDifferentiatorEstimator) f64 {
        return self.in_phase[0];
    }

    pub fn quadrature(self: *const DualDifferentiatorEstimator) f64 {
        return self.quadrt[0];
    }

    pub fn detrended(self: *const DualDifferentiatorEstimator) f64 {
        return self.detrend[0];
    }

    pub fn smoothed(self: *const DualDifferentiatorEstimator) f64 {
        return self.wma_smoothed[0];
    }

    pub fn update(self: *DualDifferentiatorEstimator, sample: f64) void {
        ...
        if (self.is_primed) {
            ...
            const discriminator = smoothed_q * (smoothed_ip - self.smoothed_in_phase_previous) -
                smoothed_ip * (smoothed_q - self.smoothed_quadrature_previous);
            ...
            const period_new = two_pi * (smoothed_ip * smoothed_ip + smoothed_q * smoothed_q) / discriminator;
            ...
            const period_previous = self._period;
            const period_new = two_pi * (smoothed_ip * smoothed_ip +
                smoothed_q * smoothed_q) / discriminator;
            ...
            if (self.smoothing_length_plus_3ht_length_min2 < self._count) { // count > 23, i.e. count >= 24
                self._period = self.emaPeriodStep(self._period, period_previous);
                self.is_primed = true;
            }
        }
    ...
    fn emaQuad(self: *const DualDifferentiatorEstimator, value: f64, previous: f64) f64 {
        return self.alpha_ema_quadrature_in_phase * value + self.one_min_alpha_ema_quadrature_in_phase * previous;
    }

    fn emaPeriodStep(self: *const DualDifferentiatorEstimator, value: f64, previous: f64) f64 {
...
// DD shares the same expected smoothed/detrended/quadrature/inPhase/period arrays as HD,
// but tests only verify up to specific indices due to diverging period calculations.

fn createDefault() DualDifferentiatorEstimator {
    const params = ht.CycleEstimatorParams{
        .smoothing_length = 4,
        .alpha_ema_quadrature_in_phase = 0.15,
        .alpha_ema_period = 0.15,
    };
    return DualDifferentiatorEstimator.init(&params) catch unreachable;
}

fn createWarmUp(warm_up: usize) DualDifferentiatorEstimator {
    const params = ht.CycleEstimatorParams{
        .smoothing_length = 4,
        .alpha_ema_quadrature_in_phase = 0.15,
        .alpha_ema_period = 0.15,
...
test "reference implementation: wma smoothed" {
    ...
    for (0..3) |i| {
        dde.update(input[i]);
        try testing.expect(almostEqual(0, dde.smoothed(), epsilon));
    }

    for (3..252) |i| {
        dde.update(input[i]);
        try testing.expect(almostEqual(exp[i], dde.smoothed(), epsilon));
    }

    const previous = dde.smoothed();
    dde.update(math.nan(f64));
    try testing.expect(almostEqual(previous, dde.smoothed(), epsilon));
}
...
test "reference implementation: detrended" {
    ...
    for (0..9) |i| {
        dde.update(input[i]);
        try testing.expect(almostEqual(0, dde.detrended(), epsilon));
    }

    // Only test up to index 23 — after that, period diverges from HD reference data.
    for (9..23) |i| {
        dde.update(input[i]);
        try testing.expect(almostEqual(exp[i], dde.detrended(), epsilon));
    }

    const previous = dde.detrended();
    dde.update(math.nan(f64));
    try testing.expect(almostEqual(previous, dde.detrended(), epsilon));
}
...
test "reference implementation: quadrature" {
    ...
    for (0..15) |i| {
        dde.update(input[i]);
        try testing.expect(almostEqual(0, dde.quadrature(), epsilon));
    }

    // Only test up to index 23 — after that, period diverges from HD reference data.
    for (15..23) |i| {
        dde.update(input[i]);
        try testing.expect(almostEqual(exp[i], dde.quadrature(), epsilon));
    }

    const previous = dde.quadrature();
    dde.update(math.nan(f64));
    try testing.expect(almostEqual(previous, dde.quadrature(), epsilon));
}
...
test "reference implementation: in-phase" {
    ...
    for (0..15) |i| {
        dde.update(input[i]);
        try testing.expect(almostEqual(0, dde.inPhase(), epsilon));
    }

    // Only test up to index 23 — after that, period diverges from HD reference data.
    for (15..23) |i| {
        dde.update(input[i]);
        try testing.expect(almostEqual(exp[i], dde.inPhase(), epsilon));
    }

    const previous = dde.inPhase();
    dde.update(math.nan(f64));
    try testing.expect(almostEqual(previous, dde.inPhase(), epsilon));
}
...
test "reference implementation: period" {
    ...
    // Check indices 0..17 against initial period value of 6.
    for (0..18) |i| {
        dde.update(input[i]);
        try testing.expect(almostEqual(6, dde.period(), epsilon));
    }

    // Indices 18..22: period may change as discriminator starts but Go reference
    // uses NaN expected values here (effectively untested). Just advance.
    for (18..23) |i| {
        dde.update(input[i]);
    }

    const previous = dde.period();
    dde.update(math.nan(f64));
    try testing.expect(almostEqual(previous, dde.period(), epsilon));
}

test "reference implementation: primed" {
    var dde = createDefault();
    const input = testInput();
    const lprimed = 3 + 7 * 3; // 24

    try testing.expect(!dde.primed());
...
test "period of sin input" {
    ...
    var dde = createDefault();
    for (0..512) |i| {
        dde.update(@sin(omega * @as(f64, @floatFromInt(i))));
    }
    try testing.expect(almostEqual(period_val, dde.period(), 1e0));
}
...
test "min period of sin input" {
    ...
    try testing.expect(almostEqual(@as(f64, @floatFromInt(dde.minPeriod())), dde.period(), 1.5e0));
}
...
test "max period of sin input" {
    ...
    try testing.expect(almostEqual(@as(f64, @floatFromInt(dde.maxPeriod())), dde.period(), 1e0));
}

test "constructor validation: smoothing length = 1" {
    const params = ht.CycleEstimatorParams{ .smoothing_length = 1 };
    const result = DualDifferentiatorEstimator.init(&params);
