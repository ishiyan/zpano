...
const AcpSpot = struct {
    i: usize,
    v: f64,
};

const AcpSnap = struct {
    i: usize,
    value_min: f64,
    value_max: f64,
    spots: []const AcpSpot,
...
test "AutoCorrelationPeriodogram update" {
    ...
    for (0..test_input.len) |i| {
        ...
        if (si < acp_snapshots.len and acp_snapshots[si].i == i) {
            const snap = acp_snapshots[si];
    ...
    try testing.expectEqual(acp_snapshots.len, si);
}
