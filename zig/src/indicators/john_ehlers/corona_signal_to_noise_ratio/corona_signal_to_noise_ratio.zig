...
test "CoronaSignalToNoiseRatio update" {
    ...
    for (0..test_input.len) |i| {
        const hl = makeHL(i, test_input[i]);
        const result = x.updateSample(test_input[i], hl.low, hl.high, @intCast(i));
...
test "CoronaSignalToNoiseRatio primes at bar 11" {
    ...
    for (0..test_input.len) |i| {
        const hl = makeHL(i, test_input[i]);
        _ = x.updateSample(test_input[i], hl.low, hl.high, @intCast(i));
...
test "CoronaSignalToNoiseRatio updateEntity" {
    ...
    // Update scalar
    {
        ...
        for (0..prime_count) |idx| {
            const hl = makeHL(idx, test_input[idx % test_input.len]);
            _ = x.updateSample(test_input[idx % test_input.len], hl.low, hl.high, time);
        }
