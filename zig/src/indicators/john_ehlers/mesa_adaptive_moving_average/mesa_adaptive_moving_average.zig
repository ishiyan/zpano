...
const test_expected_mama = [_]f64{
    math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),
    math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),
    math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),
    math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),
    math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),
    math.nan(f64),      82.81412500000000,  83.01016875000000,  83.20266031250000,  83.41990229687500,
...
const test_expected_fama = [_]f64{
    math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),
    math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),
    math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),
    math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),
    math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),      math.nan(f64),
    math.nan(f64),      82.81254062500000,  82.81748132812500,  82.82711080273440,  82.84193059008790,
...
test "MAMA update reference MAMA values" {
    ...
    for (0..lprimed) |i| {
        const result = mama.update(test_input[i]);
        try testing.expect(math.isNan(result[0]));
    }

    for (lprimed..test_input.len) |i| {
        const result = mama.update(test_input[i]);
        try testing.expect(almostEqual(result[0], test_expected_mama[i], 1e-9));
    }
...
test "MAMA update reference FAMA values" {
    ...
    for (lprimed..test_input.len) |i| {
        const result = mama.update(test_input[i]);
        try testing.expect(almostEqual(result[1], test_expected_fama[i], 1e-9));
    }
}
...
test "MAMA isPrimed" {
    ...
    for (0..lprimed) |i| {
        _ = mama2.update(test_input[i]);
        try testing.expect(!mama2.isPrimed());
    }

    _ = mama2.update(test_input[lprimed]);
    try testing.expect(mama2.isPrimed());
}
