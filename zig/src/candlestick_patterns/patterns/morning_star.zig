...
const morning_star_penetration_factor = 0.3;

pub fn morningStar(self: *const CandlestickPatterns) f64 {
    ...
    const threshold = b1.c + rb1 * morning_star_penetration_factor;
    const width = self.fuzz_ratio * rb1 * morning_star_penetration_factor;
    const mu_penetration = self.muGtRaw(b3.c, threshold, width);

    const confidence = operators.tProductAll(&.{mu_long1, mu_short2, mu_penetration});
