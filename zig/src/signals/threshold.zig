...
test "muAbove well above" {
    try std.testing.expect(almostEqual(muAbove(80.0, 70.0, 5.0, .sigmoid), 1.0, 0.01));
}
test "muAbove well below" {
    try std.testing.expect(almostEqual(muAbove(60.0, 70.0, 5.0, .sigmoid), 0.0, 0.01));
}
test "muAbove at threshold" {
    try std.testing.expect(almostEqual(muAbove(70.0, 70.0, 5.0, .sigmoid), 0.5, 1e-10));
}
test "muAbove zero width" {
    try std.testing.expectEqual(@as(f64, 1.0), muAbove(70.1, 70.0, 0.0, .sigmoid));
}
test "muBelow well below" {
    try std.testing.expect(almostEqual(muBelow(20.0, 30.0, 5.0, .sigmoid), 1.0, 0.01));
}
test "muBelow at threshold" {
    try std.testing.expect(almostEqual(muBelow(30.0, 30.0, 5.0, .sigmoid), 0.5, 1e-10));
}
test "complement" {
...
test "overbought high" {
    try std.testing.expect(muOverbought(85.0, 70.0, 5.0, .sigmoid) > 0.95);
}
test "oversold low" {
    try std.testing.expect(muOversold(15.0, 30.0, 5.0, .sigmoid) > 0.95);
}
test "overbought custom" {
    try std.testing.expect(almostEqual(muOverbought(80.0, 80.0, 5.0, .sigmoid), 0.5, 1e-10));
}
test "oversold custom" {
    try std.testing.expect(almostEqual(muOversold(20.0, 20.0, 5.0, .sigmoid), 0.5, 1e-10));
}
