...
pub const Ratios = struct {
    ...
    pub fn addReturn(
        ...
        if (self.periodicity_val == .annual) {
            fractional_period = fractional.frac(time_start, time_end, self.day_count_convention, false) catch return;
        } else {
            const fp_val = fractional.frac(time_start, time_end, self.day_count_convention, true) catch return;
